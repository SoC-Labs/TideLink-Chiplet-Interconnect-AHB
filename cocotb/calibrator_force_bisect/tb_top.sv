...
module tb_top #(
    ...
    // ----- Waveform dump ------------------------------------------------------
    // Compile-out the VCD dump for the bisect tests — they read state via APB
    // reads + log lines, so we don't need the 2+ GB-per-sim VCD file (and
    // having 5 parallel sims all writing to `waves.vcd` in the same cwd was
    // OOM-killing the slower ones).
`ifndef TB_NO_DUMP
    initial begin
        $dumpfile("waves.vcd");
        $dumpvars(0, tb_top);
