...
module tb_top #(
    ...
    // ── RUNTIME per-lane WHOLE-WORD skew select (deskew_handoff_lottery) ──────
    // 3 bits per lane (lane L = *_wordsel[3*L +: 3]), value 0..7 = whole-word
    // delay on that lane. cocotb sets these ONCE per trial at reset and holds
    // them constant through the whole bring-up, so the injected skew — and hence
    // the deskew anchor outcome — is DETERMINISTIC within a trial and identical
    // across POR/re-train retries (matching the KR260 observation: die_b's
    // post-deskew slice lands wrong the SAME way every retry). A trial changes
    // the vector only at its own POR, which models a PL-RELOAD re-roll of the
    // ribbon realization (the only thing that re-rolls the lottery on silicon).
    // Default '0 = zero skew (the negative-control all-deliver baseline).
    logic [3*NUM_PHY_LANES-1:0] m2s_wordsel = '0;   // master TX -> slave RX
    logic [3*NUM_PHY_LANES-1:0] s2m_wordsel = '0;   // slave  TX -> master RX

    // Per-direction lane delay: a runtime-selectable whole-word (16 pad_clk
    // cycle) offset per lane. A word-multiple delay shifts the lane's content by
    // an integer number of link words with no framing/clock-phase change, so
    // training/IDELAY still locks while the assembled 128-bit word is sheared —
    // exactly what the deskew's training->data handoff anchor must undo. This
    // replaces the sibling bench's compile-time pad_skid so ONE binary can model
    // several PL-reload realizations (see epoch_skew_rt.sv / README.md).
    epoch_skew_rt #(.LANES(NUM_PHY_LANES), .MAX_WORDS(7)) u_skew_m2s (
        .pad_clk_in   (m_pad_clk_tx),
        .pad_data_in  (m_pad_tx),
        .word_sel     (m2s_wordsel),
        .pad_clk_out  (m_pad_clk_tx_skid),
        .pad_data_out (m_pad_tx_skid)
    );

    epoch_skew_rt #(.LANES(NUM_PHY_LANES), .MAX_WORDS(7)) u_skew_s2m (
        .pad_clk_in   (s_pad_clk_tx),
        .pad_data_in  (s_pad_tx),
        .word_sel     (s2m_wordsel),
