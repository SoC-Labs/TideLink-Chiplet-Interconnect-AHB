// =============================================================================
// tb_top.sv — Ethernet-over-TideLink M1 integration bench (real subsystem).
//
// FORK of cocotb/eth_tidelink_pair/tb_top.sv (the M0 smoke). The ONE change vs
// M0: die_b's ahb_mng terminus is no longer the single `nanosoc_region_sram`
// scratch block — it is the WHOLE `ethernet_ss_ahb` subsystem attached through
// its external AHB-Lite slave port `eth_ss_0`, so a link-crossed write now
// transits the subsystem's OWN AHB matrix into eth_scratch_rx (0x3000_0000).
// See the M1 ATTACH block below (~line 900) and README.md. The rest is kept
// line-identical to M0/pair_v2 so PairV2TB / AHBSubMaster import unchanged.
//
// (Original M0/pair_v2 fork header follows.)
// FORK of cocotb/tidelink_top_pair_v2/tb_top.sv (verbatim structure, instance
// names u_master / u_slave, m_/s_ signal names — so pair_v2_common.PairV2TB and
// test_v2_xhb_window.AHBSubMaster import UNCHANGED). The ONE difference vs the
// pair_v2 harness: each die's `ahb_mng` terminus is no longer the throwaway
// `tb_ahb_bram_slave` scratch BRAM — it is a REAL ethernet-subsystem memory,
// `nanosoc_region_sram` (the exact module the ethernet subsystem instantiates
// as `u_region_eth_scratch_rx_0` in
// nanoSoC-refactor/ethernet-subsystem-ahb/build_soc/rtl/ethernet_ss_ahb.sv).
// It is a plain AHB-Lite SRAM slave: nanosoc_region_sram -> sl_ahb_sram ->
// cmsdk_ahb_to_sram + cmsdk_fpga_sram (the latter two already compiled by
// flists/tidelink_fpga_v2.flist, so the only NEW RTL is the two SoC Labs
// wrapper files, added by this bench's Makefile).
//
// This is the M0 milestone of docs/ETHERNET_CHIPLET_INTEGRATION.md (Shape B,
// "subsystem memory alone behind ahb_mng", scaffold §3): prove that an AHB
// write entering die_a's peer aperture (ahb_sub, 0x4000_0000+off) crosses the
// link and lands BYTE-EXACT in an ethernet-subsystem scratch RAM attached
// behind die_b's ahb_mng port. cmsdk_ahb_to_sram drives HREADYOUT=1'b1 /
// HRESP=1'b0 constant (identical timing to the reference BRAM terminus), so the
// hready<->hresp comb-loop caveat (arch §3d) does not arise.
//
...
`endif
    ...
    //   AXI->AHB -> peer ahb_mng -> the per-die ETH-SCRATCH terminus below, and
    //   the read-back returns byte-exact. Mirrors
    ...
    tidelink_top #(
        ...
        // AHB Manager — terminated by an ethernet-subsystem scratch RAM
        // (u_m_eth_scratch below): the far-side terminus of the peer's XHB500
        // window (B->A direction; idle in the M0 smoke).
    ...
    tidelink_top #(
        ...
        // AHB Manager — terminated by an ethernet-subsystem scratch RAM
        // (u_s_eth_scratch below): the far-side terminus of the MASTER's XHB500
        // window, where a master ahb_sub write into aperture 0x4000_0000 lands
        // (THE M0 datapath under test).
    ...
    // =========================================================================
    // AHB-Mng terminus (THE M1 ATTACH — real ethernet_ss_ahb via eth_ss_0).
    // ------------------------------------------------------------------------
    // M1 (first substep of docs/ETHERNET_CHIPLET_INTEGRATION.md M1, per the M0
    // README "Next steps"): replace the M0 scratch-RAM terminus with the WHOLE
    // ethernet subsystem `ethernet_ss_ahb` (nanoSoC-refactor/ethernet-subsystem-
    // ahb/build_soc/rtl/ethernet_ss_ahb.sv), attached to die_b's ahb_mng
    // through the subsystem's EXTERNAL AHB-Lite slave port `eth_ss_0`. A remote
    // access arriving on die_b's ahb_mng now transits the subsystem's OWN AHB
    // matrix (eth_ss_interconnect) — surfacing the real bus-contract behaviour
    // (matrix hready wait-states, hprot[3:0]/hsize semantics, burst handling)
    // that arch §3b/§9 flag, which the M0 single-slave BRAM could not.
    //
    // eth_ss_0 is a matrix MANAGER-receiving port (NO hsel, NO separate
    // hreadyout): the external master drives addr/data + control IN
    // (haddr/htrans/hwrite/hsize/hburst/hprot/hwdata/hmastlock), the subsystem
    // returns hrdata/hready/hresp OUT. This maps ONE-TO-ONE onto tidelink_top's
    // `ahb_mng` manager port (arch §3a) — cleaner than the M0 attach: no HSEL
    // tie, no HREADYOUT. HPROT is truncated 7->4 (ahb_mng hprot[6:0] ->
    // eth_ss_0_hprot[3:0], arch §3b); ahb_mng has no hmastlock -> tie 0.
    //
    // SIMPLIFICATIONS FOR THIS PASSIVE-SLAVE BRING-UP (all documented, see
    // README §"Simplifications"):
    //  (1) sys_fclk <- hclk. The subsystem's PRMU sets SYS_HCLK = SYS_FCLK
    //      (CLKGATE_PRESENT=0, slcorem0p_prmu.v:93), so the whole subsystem +
    //      eth_ss_0 run in the SAME hclk domain as die_b's ahb_mng manager ->
    //      synchronous attach, NO CDC (matches arch §3c: the ahb_mng<->SoC
    //      handoff is entirely hclk-domain, the XHB500 async FIFO having already
    //      resynchronised the remote access into hclk).
    //  (2) MII TX/RX + RTC domains driven by benign free-running 25 MHz clocks;
    //      the MAC transmit/receive datapath is NOT exercised (no PHY, M1 no-PHY
    //      frame relay — arch §5). Only the AHB/memory layer is under test.
    //  (3) The Cortex-M0+ core boots but is PARKED: IMEM is preloaded with a
    //      Thumb branch-to-self (image_spin.hex), so after bootrom the CPU spins
    //      and never issues AHB traffic to eth_scratch_rx (0x3000_0000). The
    //      byte-exact check independently guards against any CPU interference.
    //      (A true "core held in reset" passive slave is NOT possible here: the
    //      subsystem's HCLK/HRESETn are generated by the CPU PRMU from
    //      sys_sysresetn, so holding the core in reset also holds the matrix +
    //      memories in reset. Parking via the spin image is the honest
    //      equivalent — documented as SIMPLIFICATION 3.)
    //  (4) eth_ss_1 (2nd external slave) tied idle; CPU sideband / DBGAHB / IOP
    //      / UART / MDIO tied off.
    //
    // Only die_b carries the subsystem (die_a -> die_b relay direction). die_a's
    // ahb_mng (the B->A path) is an idle always-ready sink here.
    // =========================================================================

    // ---- die_a ahb_mng (B->A): idle always-ready sink (no B->A traffic) -----
    assign m_mng_hready = 1'b1;
    assign m_mng_hresp  = 1'b0;
    assign m_mng_hrdata = '0;

    // ---- die_b ethernet-subsystem clocks -----------------------------------
    wire  eth_sys_fclk = hclk;                      // SIMP 1: SYS_HCLK == hclk
    logic eth_mtx_clk = 1'b0; always #20 eth_mtx_clk = ~eth_mtx_clk;  // 25 MHz
    logic eth_rtc_clk = 1'b0; always #20 eth_rtc_clk = ~eth_rtc_clk;  // 25 MHz

    // =========================================================================
    // MII EXTERNAL LOOPBACK  (eth_ptp_chain: LIFTS Shape-A SIMPLIFICATION 2)
    // =========================================================================
    // Shape-A drove mrxd_i/mrxdv_i to constant 0 and ran mtx_clk/mrx_clk as two
    // INDEPENDENT 25 MHz oscillators: the MAC datapath was never exercised, so
    // HA1588 — which timestamps on MII frame events — could never capture.
    //
    // Here the MII TX pins are looped straight back onto the MII RX pins, which
    // is exactly what an external PHY in loopback (or a crossover to itself)
    // presents. This is the SMALLEST HONEST stimulus that exercises the REAL
    // MII timestamp path end to end:
    //
    //     MAC TX FSM -> mtxd_o/mtxen_o -> [loop] -> mrxd_i/mrxdv_i -> MAC RX FSM
    //                                       |
    //                       HA1588 ptp_event_detector taps BOTH edges
    //
    // Nothing is faked: the frame is DMA'd out of memory by the MAC's own
    // Wishbone/AHB master, serialised nibble-by-nibble by the real TX FSM, and
    // re-parsed by the real RX FSM. HA1588 sees genuine MII activity.
    //
    // ONE clock, not two: a real loopback is inherently source-synchronous —
    // the returning data is clocked by the same oscillator that launched it.
    // Driving mrx_clk from an independent oscillator (as Shape-A did) would
    // make the loopback a CDC with no phase relationship and corrupt every
    // nibble. mrx_clk_i := mtx_clk_i is the physically faithful choice.
    wire eth_mrx_clk = eth_mtx_clk;

    wire [3:0] eth_mtxd;      // MAC -> loop
    wire       eth_mtxen;
    wire       eth_mtxerr;

    wire [3:0] eth_mrxd  = eth_mtxd;    // loop -> MAC
    wire       eth_mrxdv = eth_mtxen;
    wire       eth_mrxerr = 1'b0;       // clean loop: never inject a PHY error

    // Carrier-sense / collision must track the loopback or the MAC's TX FSM
    // will either never see carrier or will declare a false collision:
    //   mcrs_i  = carrier present  -> asserted while the loop carries a frame
    //   mcoll_i = collision        -> tied 0 (full-duplex loopback, no collision)
    wire       eth_mcrs  = eth_mtxen;
    wire       eth_mcoll = 1'b0;

    // ---- HA1588 RTC witness (grandmaster time, observed non-intrusively) ----
    wire [31:0] eth_rtc_ptp_ns;
    wire [47:0] eth_rtc_ptp_sec;
    wire        eth_rtc_one_pps;

    // ---- MII frame observer + RECORDER (independent, non-bus witness) -------
    // Counts frames seen on the loop so the test can prove a frame PHYSICALLY
    // moved, independently of any register read. Rising edge of mtxen = SoF.
    //
    // It ALSO records the first MII_CAP_BYTES bytes of the most recent frame,
    // reassembled from nibbles (MII is LOW NIBBLE FIRST). This is deliberate
    // instrument-first discipline: the DMA-buffer -> wire BYTE ORDER of the
    // OpenCores MAC is not something to guess at. With this recorder the test
    // PRINTS what is physically on the wire (preamble/SFD, DA, SA, EtherType)
    // and can confirm the 0x88F7 PTP EtherType actually appears -- rather than
    // inferring byte order from a silent no-capture.
    localparam int MII_CAP_BYTES = 64;

    integer mii_tx_frames  = 0;
    integer mii_tx_nibbles = 0;
    logic   mtxen_q = 1'b0;

    logic [7:0] mii_cap_mem [0:MII_CAP_BYTES-1];
    integer     mii_cap_len   = 0;   // bytes captured for the current frame
    logic       mii_cap_phase = 1'b0; // 0 = expecting low nibble
    logic [3:0] mii_cap_lo    = 4'h0;

    always @(posedge eth_mtx_clk) begin
        mtxen_q <= eth_mtxen;
        if (eth_mtxen && !mtxen_q) begin           // start of frame
            mii_tx_frames <= mii_tx_frames + 1;
            mii_cap_len   <= 0;
            mii_cap_phase <= 1'b0;
        end
        if (eth_mtxen) begin
            mii_tx_nibbles <= mii_tx_nibbles + 1;
            if (mii_cap_phase == 1'b0) begin
                mii_cap_lo    <= eth_mtxd;          // low nibble first
                mii_cap_phase <= 1'b1;
            end else begin
                if (mii_cap_len < MII_CAP_BYTES) begin
                    mii_cap_mem[mii_cap_len] <= {eth_mtxd, mii_cap_lo};
    ...
    // Flattened mirror of the capture buffer. cocotb can index `mii_cap_mem`
    // only awkwardly across simulators; a packed vector is read as one value.
    logic [MII_CAP_BYTES*8-1:0] mii_cap_flat;
    integer ci;
    always @(*) begin
        for (ci = 0; ci < MII_CAP_BYTES; ci = ci + 1)
            mii_cap_flat[ci*8 +: 8] = mii_cap_mem[ci];
    end

    // ---- ahb_mng address-phase MONITOR (memory-map derivation) --------------
    // Latch the address/control die_b's ahb_mng manager actually presents to
    // eth_ss_0 on each non-IDLE address phase, so the test can DERIVE the
    // peer-window -> ahb_mng address transform empirically (rather than assume
    // it), and log it. HTRANS[1]=1 => NONSEQ(2) or SEQ(3).
    logic [31:0] s_mng_haddr_seen  = 32'hDEAD_BEEF;
    logic        s_mng_hwrite_seen = 1'b0;
    logic  [1:0] s_mng_htrans_seen = 2'b00;
    logic  [2:0] s_mng_hsize_seen  = 3'b000;
    logic  [2:0] s_mng_hburst_seen = 3'b000;
    always @(posedge hclk) begin
        if (s_mng_htrans[1] && s_mng_hready) begin
            s_mng_haddr_seen  <= s_mng_haddr;
            s_mng_hwrite_seen <= s_mng_hwrite;
            s_mng_htrans_seen <= s_mng_htrans;
            s_mng_hsize_seen  <= s_mng_hsize;
            s_mng_hburst_seen <= s_mng_hburst;
        end
    end

    // ---- die_b ahb_mng terminus = ethernet_ss_ahb (eth_ss_0) ----------------
`ifndef ETH_IMEM_HEX
  `define ETH_IMEM_HEX "image_spin.hex"
`endif
    ethernet_ss_ahb #(
        .IMEM_MEM_FPGA_IMG (`ETH_IMEM_HEX)          // SIMP 3: park the CPU
    ) u_s_eth_ss (
        .sys_fclk        (eth_sys_fclk),
        .sys_sysresetn   (s_hresetn_w),              // shares die_b reset tree
        .sys_scanenable  (1'b0),
        .sys_testmode    (1'b0),
        .sys_sysresetreq (1'b0),                     // no external reset request
        .sys_poresetn    (),
        .sys_hclk        (),
        .sys_hresetn     (),
        .cpu_0_pmuenable (1'b0),
        // CPU system-passthrough master (0x2000_0000 region) — unused; respond
        // ready/OKAY so a stray access can't wedge (spin CPU won't reach it).
        .cpu_0_haddr     (),
        .cpu_0_htrans    (),
        .cpu_0_hwrite    (),
        .cpu_0_hsize     (),
        ...
        .cpu_0_hready    (1'b1),
        .cpu_0_hresp     (1'b0),
        // eth_ss_0 <- die_b ahb_mng manager (THE ATTACH)
        .eth_ss_0_haddr     (s_mng_haddr),
        .eth_ss_0_htrans    (s_mng_htrans),
        .eth_ss_0_hwrite    (s_mng_hwrite),
        .eth_ss_0_hsize     (s_mng_hsize),
        .eth_ss_0_hburst    (s_mng_hburst),
        .eth_ss_0_hprot     (s_mng_hprot[3:0]),      // AHB5 7->4 truncation
        .eth_ss_0_hwdata    (s_mng_hwdata),
        .eth_ss_0_hmastlock (1'b0),                  // ahb_mng has no hmastlock
        .eth_ss_0_hrdata    (s_mng_hrdata),
        .eth_ss_0_hready    (s_mng_hready),
        .eth_ss_0_hresp     (s_mng_hresp),
        // eth_ss_1 idle
        ...
        .cpu_0_sleepdeep (),
        // DBGAHB idle
        .cpu_0_dbgahb_slvaddr  (32'h0),
        .cpu_0_dbgahb_slvwdata (32'h0),
        .cpu_0_dbgahb_slvtrans (2'b00),
        .cpu_0_dbgahb_slvwrite (1'b0),
        .cpu_0_dbgahb_slvsize  (2'b10),
        .cpu_0_dbgahb_slvrdata (),
        .cpu_0_dbgahb_slvready (),
        .cpu_0_dbgahb_slvresp  (),
        // M0+ single-cycle I/O port idle
        .cpu_0_ioaddr    (),
        .cpu_0_iotrans   (),
        .cpu_0_iowrite   (),
        .cpu_0_iosize    (),
        ...
        .cpu_0_iordata   (32'h0),
        .cpu_0_iomaster  (),
        // RTC / PTP — the HA1588 RTC free-runs on this clock. Exported here so
        // the tb can WITNESS the grandmaster time non-intrusively (and so the
        // test can prove a captured timestamp is consistent with it).
        .rtc_clk         (eth_rtc_clk),
        .rtc_time_ptp_ns (eth_rtc_ptp_ns),
        .rtc_time_ptp_sec(eth_rtc_ptp_sec),
        .rtc_time_one_pps(eth_rtc_one_pps),
        // MII — EXTERNAL LOOPBACK (Shape-A SIMPLIFICATION 2 lifted)
        .mtx_clk_i       (eth_mtx_clk),
        .mtxd_o          (eth_mtxd),
        .mtxen_o         (eth_mtxen),
        .mtxerr_o        (eth_mtxerr),
        .mrx_clk_i       (eth_mrx_clk),
        .mrxd_i          (eth_mrxd),
        .mrxdv_i         (eth_mrxdv),
        .mrxerr_i        (eth_mrxerr),
        .mcoll_i         (eth_mcoll),
...
// The pair_v2 harness's `tb_ahb_bram_slave` scratch-BRAM terminus is REMOVED in
// this fork — both ahb_mng ports now terminate in the ethernet-subsystem
// `nanosoc_region_sram` (instantiated above). Its RTL comes from the
// ethernet-subsystem-ahb repo via this bench's Makefile.
