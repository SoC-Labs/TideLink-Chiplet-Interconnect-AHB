// =============================================================================
// tb_top.sv — Ethernet-over-TideLink M0 integration smoke.
//
...
`endif
    ...
    // =========================================================================
    // AHB-Mng ETHERNET-SCRATCH terminus, one per die (THE M0 ATTACH).
    // ------------------------------------------------------------------------
    // A remote access arriving on a die's ahb_mng (the far end of the peer's
    // XHB500 transparent window) lands in a REAL ethernet-subsystem memory:
    // `nanosoc_region_sram` — the module instantiated as
    // `u_region_eth_scratch_rx_0` / `_tx_0` in
    // nanoSoC-refactor/ethernet-subsystem-ahb/build_soc/rtl/ethernet_ss_ahb.sv.
    // It wraps sl_ahb_sram -> cmsdk_ahb_to_sram + cmsdk_fpga_sram (byte-lane
    // BRAM0..3), a zero-wait AHB-Lite slave (HREADYOUT=1'b1 / HRESP=1'b0
    // constant — same timing contract as the pair_v2 reference BRAM terminus,
    // so no hready<->hresp comb loop).
    //
    // RAM_ADDR_W=14 => 16 KB (4096 words), matching the eth scratch default.
    // The region takes the full 32-bit HADDR and slices HADDR[13:0] internally,
    // so the 0x4000_0000 aperture bits are ignored and a peer-window offset
    // `off` (< 16 KB) selects word (off>>2). HSEL is tied high (this is a
    // dedicated single-slave terminus; HTRANS gates the capture). HPROT takes
    // the low 4 bits of ahb_mng's AHB5 hprot[6:0] (arch §3b width note).
    //
    // BOTH dies present an eth-scratch to the far die (symmetric). The M0 smoke
    // exercises die_a -> die_b (master peer window -> u_s_eth_scratch); the
    // master-side u_m_eth_scratch is the B->A terminus, idle here, ready for a
    // future bidirectional test.
    // =========================================================================
    nanosoc_region_sram #(
        .SYS_ADDR_W (SYS_ADDR_W),
        .SYS_DATA_W (SYS_DATA_W),
        ...
        .HTRANS    (m_mng_htrans),
        .HSIZE     (m_mng_hsize),
        .HPROT     (m_mng_hprot[3:0]),
        .HWRITE    (m_mng_hwrite),
        .HREADY    (1'b1),
        .HWDATA    (m_mng_hwdata),
        .HREADYOUT (m_mng_hready),
        .HRESP     (m_mng_hresp),
        .HRDATA    (m_mng_hrdata)
    );
    ...
    nanosoc_region_sram #(
        ...
        .HTRANS    (s_mng_htrans),
        .HSIZE     (s_mng_hsize),
        .HPROT     (s_mng_hprot[3:0]),
        .HWRITE    (s_mng_hwrite),
        .HREADY    (1'b1),
        .HWDATA    (s_mng_hwdata),
        .HREADYOUT (s_mng_hready),
        .HRESP     (s_mng_hresp),
        .HRDATA    (s_mng_hrdata)
    );
