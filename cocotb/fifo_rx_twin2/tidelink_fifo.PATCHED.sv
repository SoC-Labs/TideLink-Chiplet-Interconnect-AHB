...
module tidelink_fifo #(
    ...
    tidelink_fifo_mem #(
        ...
        // TWIN 2 FIX (F10): this is the RX FIFO — its AHB slave is CPU-READ-ONLY
        // (received data is committed by the FC adapter via the fc_wr_* port, per
        // tidelink_top.sv "FC adapter RX writes + CPU reads"). Reject AHB WRITES so
        // a stray/bulk write to the read-only RX window cannot walk the FC-shared
        // write_ptr. No software writes this aperture (grep evidence in the
        // disposition); the TX aperture 0x84000000 is a separate datapath.
        .ENABLE_AHB_WRITE (0)
    ) u_fifo_mem (
        .hclk                   (hclk),
        .hresetn                (hresetn),
