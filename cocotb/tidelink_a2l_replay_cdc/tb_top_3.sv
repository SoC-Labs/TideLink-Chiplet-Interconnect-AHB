// ---------------------------------------------------------------------------
// tb_top_3.sv  --  unit testbench for the W-channel (0x81) a2l replay FIFO node
// WlinkGenericFCReplayV2_3 (37-bit data / 6-bit ptr / depth-32).
//
...
module tb_top (
    ...
    // App-side (write) interface — 37-bit data
    input  logic        app_enable,
    input  logic [36:0] app_data,
    ...
    output logic [36:0] link_data,
    output logic        link_valid,
    output logic        link_empty
);

    WlinkGenericFCReplayV2_3 dut (
        .app_clk          (app_clk),
        .app_reset        (app_reset),
