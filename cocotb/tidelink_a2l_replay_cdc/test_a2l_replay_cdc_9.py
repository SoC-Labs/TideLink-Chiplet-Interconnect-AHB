"""R-channel (0x84) a2l replay-FIFO CDC REPRODUCE — the READ-path analogue of
test_a2l_replay_cdc_3 for the R (read-data) node WlinkGenericFCReplayV2_9
...
src/rtl/local_overrides/WlinkGenericFCReplayV2_9.v -- the file does not exist.
...
(flists/tidelink_top_full_asic_v2.flist takes _9 and _9 from deps/).
...
and the read path never was.
  ...
  deps _9 (what ships, and what this compiles):
...
to lock out and it passes for the wrong reason. Measured: deps _9
...
Expected results:
  ...
  once an override _9 is written     -> PASS   (self-heal)
