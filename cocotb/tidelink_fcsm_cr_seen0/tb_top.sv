...
module tb_top #(
    ...
    // ── F6 split-clock (async 2-oscillator) variant ───────────────────────────
    // When TB_SPLIT_CLK is defined the SLAVE die runs on independent
    // s_hclk / s_ref_clk oscillators (driven by cocotb at a ppm offset), so the
    // two dies' calibrator S_HOLD/S_VALIDATE windows do NOT stay phase-locked —
    // the bilateral CR-crossing rendezvous can then genuinely miss, which is the
    // async race the shared-clock tb hides (docs/I1_REPRO_ASYNC_LADDER.md, F6).
    // When NOT defined the slave shares hclk/ref_clk (bit-identical to the
    // original shared-clock tb, so rungs 0-1 are unchanged).
`ifdef TB_SPLIT_CLK
    logic s_hclk    = 1'b0;   // cocotb-driven, independent ppm-offset oscillator
    logic s_ref_clk = 1'b0;   // cocotb-driven, independent ppm-offset oscillator
`else
    wire  s_hclk    = hclk;
    wire  s_ref_clk = ref_clk;
`endif
    ...
    // Raw skid outputs, fed into the F14 error injector (u_inj_*).
    wire                          m_pad_clk_tx_sk, s_pad_clk_tx_sk;
    wire [NUM_PHY_LANES-1:0]      m_pad_tx_sk,     s_pad_tx_sk;
    // FINAL post-injection pads consumed by the peer RX.
    ...
    pad_skid #(
        ...
        .pad_clk_out  (m_pad_clk_tx_sk),
        .pad_data_out (m_pad_tx_sk)
    );
    ...
    pad_skid #(
        ...
        .pad_clk_out  (s_pad_clk_tx_sk),
        .pad_data_out (s_pad_tx_sk)
    );

    // ─────────────────────────────────────────────────────────────────────────
    // F14 ERROR INJECTORS on BOTH directions (post-skid, pre-RX). HARNESS ONLY.
    // Default passthrough (all controls decl-init to "no injection") so this
    // build is bit-identical to the un-injected harness until cocotb deposits
    // u_inj_<dir>.inj_en / lane_mask / mode / clk_kill.
    //   u_inj_m2s : master->slave pads  -> corrupts the SLAVE's RX
    //   u_inj_s2m : slave->master pads  -> corrupts the MASTER's RX
    // ─────────────────────────────────────────────────────────────────────────
    err_inject #(.LANES(NUM_PHY_LANES)) u_inj_m2s (
        .pad_clk_in   (m_pad_clk_tx_sk),
        .pad_data_in  (m_pad_tx_sk),
        .pad_clk_out  (m_pad_clk_tx_skid),
        .pad_data_out (m_pad_tx_skid)
    );
    err_inject #(.LANES(NUM_PHY_LANES)) u_inj_s2m (
        .pad_clk_in   (s_pad_clk_tx_sk),
        .pad_data_in  (s_pad_tx_sk),
...
`endif
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // I1 cr_seen=0 repro (docs/I1_REPRO_ASYNC_LADDER.md) — F4 knob.
    // Shrink the un-bypassed calibrator's S_HOLD dwell (and optionally the
    // S_VALIDATE window) via hierarchical defparam, so a COLD un-bypassed
    // bring-up is feasible in sim WITHOUT forcing tb_early_exit_force_q (which
    // would sever the cal<-cr_pkt_seen coupling). S_VALIDATE still gates on the
    // real sideband cr_pkt_seen. Default HOLD_CYCLES = 8*128*64 = 65536 link
    // cycles (~1 ms) is left untouched unless TB_CAL_HOLD_CYCLES is defined.
    // Only valid under TIDELINK_PHY_V2 (the u_calibrator instance the harness
    // and this repro use). See axi_chiplet_controller.sv:5819.
    // ─────────────────────────────────────────────────────────────────────────
`ifdef TB_CAL_HOLD_CYCLES
    defparam u_master.u_chiplet_controller.u_calibrator.HOLD_CYCLES = `TB_CAL_HOLD_CYCLES;
    defparam u_slave.u_chiplet_controller.u_calibrator.HOLD_CYCLES  = `TB_CAL_HOLD_CYCLES;
`endif
`ifdef TB_CAL_VAL_TIMEOUT
    defparam u_master.u_chiplet_controller.u_calibrator.VALIDATION_TIMEOUT = `TB_CAL_VAL_TIMEOUT;
    defparam u_slave.u_chiplet_controller.u_calibrator.VALIDATION_TIMEOUT  = `TB_CAL_VAL_TIMEOUT;
`endif
`ifdef TB_CAL_VAL_TIMEOUT_TO_DONE
    // Force VAL_TIMEOUT_TO_DONE to the given value (0 restores the pre-2026-06-18
    // no-give-up behaviour so a starved S_VALIDATE holds cal_done=0 like the
    // FPGA `else calibrator, matching the silicon 4-tuple more closely).
    defparam u_master.u_chiplet_controller.u_calibrator.VAL_TIMEOUT_TO_DONE = `TB_CAL_VAL_TIMEOUT_TO_DONE;
    defparam u_slave.u_chiplet_controller.u_calibrator.VAL_TIMEOUT_TO_DONE  = `TB_CAL_VAL_TIMEOUT_TO_DONE;
`endif
