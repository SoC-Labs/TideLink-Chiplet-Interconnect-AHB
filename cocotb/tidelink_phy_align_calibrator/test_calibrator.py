...
# ---------------------------------------------------------------------------
# (b) THE INVARIANT — cal_done on VALIDATION TIMEOUT without a lock
# ---------------------------------------------------------------------------
@cocotb.test()
async def test_validate_timeout_without_lock(dut):
    ...
    # Phase 1: lanes lock TRANSIENTLY during the sweep so every lane records
    # a passing point (any_pass / run) -> sweep_success=1 -> no fault ->
    # S_FINISH->S_HOLD. This is the training-pattern "false positive": the
    # lane_checker saw matching training bytes, but the latched (slip,phase)
    # does NOT decode real data.
    dut.lane_locked.value = 0xFF
    # cr/crack held LOW the whole test: the FCSM never decodes a real packet
    # (exactly the silicon condition — training passed, real data does not).
    dut.cr_pkt_seen_i.value = 0
    dut.crack_pkt_seen_i.value = 0
    await _trigger(dut)

    # Let the sweep + finalize complete, then enter S_HOLD.
    reached_hold = await _wait_state(dut, S_HOLD, FULL_SWEEP_CYCLES + 64)
    assert reached_hold, "never reached S_HOLD (sweep did not succeed?)"
    dut._log.info("[invariant] reached S_HOLD with sweep_success=1 (no lane faulted)")

    # Phase 2: the peer switches to real data -> our lane_checker drops.
    # lane_locked goes to 0x00 and STAYS there (the latched phase cannot
    # decode the real data). This is the silicon condition exactly.
    dut.lane_locked.value = 0x00

    # Wait until we enter S_VALIDATE (after HOLD_CYCLES).
    reached_val = await _wait_state(dut, S_VALIDATE, HOLD_CYCLES + 64)
    assert reached_val, "never reached S_VALIDATE"
    dut._log.info("[invariant] reached S_VALIDATE; lane_locked=0x00, cr/crack=0")

    # Now run out the validation timeout and capture cal_done at S_DONE.
    reached_done = await _wait_state(dut, S_DONE, VALIDATION_TIMEOUT + 64)
    assert reached_done, "S_VALIDATE never timed out to S_DONE"
    ...
    fault    = int(dut.lane_fault.value)
    dut._log.info(
        f"[invariant] S_DONE via VALIDATION TIMEOUT: cal_done={cal_done} "
        f"lane_locked=0x{locked:02x} lane_fault=0x{fault:02x}")

    # This is the BUG REPRODUCTION assertion. If cal_done==1 while
    # lane_locked==0 and cr/crack never fired, the calibrator declared DONE
    # purely on the timeout — the silicon symptom.
    bug_reproduced = (cal_done == 1 and locked == 0)
    if bug_reproduced:
        dut._log.error(
            "[invariant] BUG REPRODUCED: cal_done=1 with lane_locked=0x00 "
            "AND cr/crack never asserted -> DONE was declared on TIMEOUT, "
            "not on a genuine lock.")
    else:
        dut._log.info(
            "[invariant] cal_done was NOT asserted without a lock "
            "(guard present?).")

    # Cross-check the proposed guard's status bit if the RTL exposes it.
    # validation_timed_out distinguishes a timeout-DONE from a real-lock DONE.
    timed_out = _opt_int(dut, "validation_timed_out")

    # The test's VERDICT-bearing assertion:
    #   * Without a guard, this documents the defect: we EXPECT the bug, and
    #     additionally require that SOME observable lets SW tell timeout from
    #     real lock. If neither lock NOR a status bit is available, that is the
    #     unsafe condition the task is about.
    if cal_done == 1 and locked == 0:
        # cal_done asserted without a lock. This is only SAFE if a distinct
        # status bit flags it as a timeout so SW/FCSM does not trust it.
        assert timed_out == 1, (
            "DEFECT: cal_done=1 with no lock and NO validation_timed_out flag "
            "-> SW cannot distinguish a timeout-DONE from a real-lock DONE "
            "(this is the silicon hang).")
        dut._log.info(
            "[invariant] cal_done=1 on timeout BUT validation_timed_out=1 "
            "flags it -> SW can avoid trusting the link. GUARD OK.")
    else:
        dut._log.info(
            "[invariant] cal_done gated by lock -> no unsafe DONE. GUARD OK.")
