"""Bug N11 alt-hypothesis — does SIMULTANEOUS POR sidestep the
asymmetric-power-up cascade observed on silicon build v11?
...
@cocotb.test()
async def test_22_simultaneous_por_cascade(dut):
    """Both dies POR at the same cycle. Pure-RTL, no APB stimulus, no
    forced peer-status pins. Expect both dies to reach role_lock=1 AND
    (train_ok=1 OR slave-side ST_NEGO_DONE-lost-then-resync). The minimum
    bar is: master train_ok=1 AND no die parked in ST_ERROR / ST_TRAIN_FAIL.
    """
    log = dut._log
    ...
    # Pass criteria for the alt-hypothesis: simultaneous POR yields a
    # clean autoneg → training walk for the winner. The loser may park
    # in ST_NEGO_DONE-lost (state 5) — that's a deliberate FSM design
    # choice — but it MUST have latched role_locked.
    failures = []
    if m_role != 1:
        failures.append(f"master role_locked={m_role} (expected 1)")
    if s_role != 1:
        failures.append(f"slave role_locked={s_role} (expected 1)")
    if m_state == ST_ERROR:
        failures.append(f"master in ST_ERROR")
    if s_state == ST_ERROR:
        failures.append(f"slave in ST_ERROR")
    if m_train_fail == 1:
        failures.append(f"master train_fail=1")
    if s_train_fail == 1:
        failures.append(f"slave train_fail=1")
    # Minimum bar: master (the winner under the asymmetric prio defaults
    # injected by tb_top) must reach train_ok. The slave's path through
    # ST_NEGO_DONE-lost does NOT enter ST_TRAIN_*, so we don't require
    # s_train_ok=1 — but if the slave HAS reached ST_TRAIN_*, train_fail
    # must be 0 (covered above).
    if m_train_ok != 1:
        failures.append(
            f"master train_ok={m_train_ok} (expected 1) — state="
            f"{_state_name(m_state)}"
        )

    if failures:
        raise AssertionError(
            "Simultaneous-POR cascade hypothesis: FAILED.\n"
            "  Bug N11 (Wlink lane_mask reads TL_ID magic) bites the "
            "symmetric POR case too — RTL fix on the parallel agent's "
    ...
    log.info(
        "PASS: simultaneous-POR cascade hypothesis confirmed — "
        "the silicon cascade IS asymmetric-POR-only. Deploy-script "
        "tweak (parallel fpga_manager flash) should sidestep Bug N11 "
        "for symmetric bring-up, but a real fix is still needed for "
        "any asymmetric POR (warm-restart, partial-reconfig, etc.)."
    )
