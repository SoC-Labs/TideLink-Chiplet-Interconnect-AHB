...
module tb_top #(
    ...
    // -------------------------------------------------------------------------
    // Clocks and resets — DRIFT variant
    //
    // Each side gets its OWN hclk and ref_clk (cocotb drives them with
    // slightly different periods to emulate inter-board MMCM PPM drift on
    // two separate PYNQ-Z2 boards). On HW each board's clock comes from its
    // own crystal + MMCM, so the master and slave see uncorrelated phases
    // and drift at ~250 PPM (typical XTAL tolerance). The original sim uses
    // a shared clock — this variant exposes the inter-board asynchrony as
    // an env knob for the calibrator probe debug.
    // -------------------------------------------------------------------------
    logic hclk_m    = 1'b0;
    logic hclk_s    = 1'b0;
    logic ref_clk_m = 1'b0;
    logic ref_clk_s = 1'b0;
    logic poresetn  = 1'b0;
    logic hresetn   = 1'b0;

    // Back-compat aliases so legacy code paths (e.g. reset sequencing,
    // pad-skid timing) that referenced `hclk` / `ref_clk` still resolve —
    // by convention these track the MASTER's clock domain.
    wire  hclk     = hclk_m;
    wire  ref_clk  = ref_clk_m;
    ...
    tidelink_top #(
        ...
        // Wlink PLL reference — per-side (DRIFT variant)
        .user_ref_clk      (ref_clk_m),
        ...
        // PHC — tied off (PTP not exercised); use per-side hclk to keep PHC
        // capture path matched to its local domain.
        .phc_clk                    (hclk_m),
        .phc_resetn                 (hresetn),
    ...
    tidelink_top #(
        ...
        // PHC — per-side hclk (DRIFT variant)
        .phc_clk                    (hclk_s),
        .phc_resetn                 (hresetn),
        .phc_nanoseconds            (30'h0),
        .phc_seconds                (48'h0),
