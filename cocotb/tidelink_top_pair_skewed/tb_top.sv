...
module tb_top #(
    ...
    // -------------------------------------------------------------------------
    // SKEWED VARIANT: asymmetric per-lane skid emulates ribbon-cable skew
    // between two PYNQ-Z2 boards. On real HW the per-lane trace lengths and
    // FT2232H clock recovery latency differ from lane to lane, so the eye
    // centre per-lane is NOT uniformly at (slip=0, phase=0). This testbench
    // sets a hand-picked asymmetric pattern so that the calibrator's bias-
    // to-(0,0) fix (commit b5f92e8) is forced to fall through to the original
    // 128-point sweep on the lanes that don't lock at the bias point — which
    // is when the per-lane race-to-tie best-of-sweep latch bug (Agent A's S1)
    // re-surfaces.
    //
    // Values chosen so that no single lane is at (0,0) on both directions —
    // i.e. (0,0) is NOT the natural eye centre. M->S and S->M are also chosen
    // ASYMMETRIC, so master's RX and slave's RX see different per-lane skews.
    // -------------------------------------------------------------------------
    pad_skid #(
