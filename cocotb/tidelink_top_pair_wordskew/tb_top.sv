...
module tb_top #(
    ...
    // -------------------------------------------------------------------------
    // WORDSKEW VARIANT: per-lane skid in WHOLE-WORD (16-bit) multiples.
    // Unlike the sub-word skewed variant, these delays are multiples of the
    // 16-bit deserializer word, so each lane's bit-phase is UNCHANGED (the
    // per-lane training byte still lands in the same bit position every word)
    // -> each lane locks individually (SWI_LANE_STATUS low byte -> 0xff) -
    // but lanes are offset from each other by whole words. Lane 0 is the
    // bus-timing reference (skid 0). With cross-lane deskew ABSENT, the
    // combinational 128-bit assembly mixes bytes from different master TX
    // words once varying data flows in DATA mode -> framer never finds SOP.
    // This reproduces the HW deskew fingerprint: lanes "locked" 0xff yet the
    // link is dead. During TRAINING (constant per-lane patterns) the word
    // skew is invisible, matching "training fine, data mode corrupts".
    // M->S and S->M asymmetric so each RX sees a different per-lane word skew.
    // -------------------------------------------------------------------------
    pad_skid #(
    ...
    // ----- Waveform dump ------------------------------------------------------
    // Disabled by default in the skewed variant: a full-hierarchy VCD over the
    // calibrator sweep window blows past 6+ GB and dominates wall time. Re-
    // enable with `+define+TB_VCD_ENABLE` if interactive debugging is needed.
`ifdef TB_VCD_ENABLE
    initial begin
        $dumpfile("waves.vcd");
        $dumpvars(0, tb_top);
