...
OFF_R8_SYNC_DET         = 0x114     # slot 5: [31:16] sync_detected sat-cnt (SoC Labs 2026-06-08), [15:0] ecc_corrupted (dead)
...
APB_R8_SYNC_DET         = APB_TIDELINK_BASE + OFF_R8_SYNC_DET # 0x2114
...
async def run_bringup_through_phase1(tb):
    ...
    # bypass the 500k-cycle sim budget is exhausted before S_VALIDATE expires.
...
# ===========================================================================
# SoC Labs 2026-06-08 — STEADY-STATE cross-lane-skew data proof.
#
# test_08 sends ONE packet shortly after training drops, so its alignment could
# in principle ride the BOOTSTRAP (training-edge) prime. This test sends a FIRST
# packet, drains the slave RX FIFO, idles long enough for SYNC words to flow and
# the SYNC re-prime to run, then sends a SECOND distinct packet — which can only
# land if the deskew re-aligns on the SYNC word in STEADY STATE (the training
# edge is long gone). Under the wordskew tb (lanes offset 0/16/32 bit-periods)
# this fails with the training-edge-only deskew and passes with the SYNC-anchored
# deskew. Both packets are gated, so a single mis-framed 128-bit word corrupts a
# payload and trips the assert.
# ===========================================================================
@cocotb.test()
async def test_16_steady_state_two_packets_under_skew(dut):
    """Two M->S packets across an idle gap; the 2nd relies on steady-state SYNC
    re-prime (not the training-edge bootstrap). Proves the SYNC-content-anchored
    deskew holds alignment under sustained cross-lane WORD skew."""
    from tidelink.packet import encode_word0, PKT_WR_REQ
    ...
    async def send_and_check(tag, payload):
        word0 = encode_word0(length=len(payload), pkt_type=PKT_WR_REQ,
                             src_id=0, dest_id=0, tag=0)
        words = [word0, 0x0] + payload
        tb.log.info(f"  [{tag}] master TX payload=[0x{payload[0]:08x}, "
                    f"0x{payload[1]:08x}]")
        await tb.ahb_tx_write_packet("m", words)
        await ClockCycles(tb.dut.hclk, 2000)
        # Read the FULL packet (0x00..0x0C) so the FIFO read_complete fires on the
        # final word (read_target_addr) and read_ptr advances past this packet —
        # otherwise the NEXT packet reads back this one's stale payload.
        s_w0 = await tb.ahb_fifo_read_word("s", 0x00)
        s_w1 = await tb.ahb_fifo_read_word("s", 0x04)
        s_w2 = await tb.ahb_fifo_read_word("s", 0x08)
        s_w3 = await tb.ahb_fifo_read_word("s", 0x0C)
        tb.log.info(f"  [{tag}] slave FIFO payload: w0=0x{s_w0:08x} "
                    f"w1=0x{s_w1:08x} w2=0x{s_w2:08x} w3=0x{s_w3:08x}")
        assert s_w2 == payload[0] and s_w3 == payload[1], (
            f"[{tag}] slave FIFO payload mismatch under word-skew: read "
            f"[0x{s_w2:08x}, 0x{s_w3:08x}], expected "
            f"[0x{payload[0]:08x}, 0x{payload[1]:08x}].")

    # Packet A — may use the bootstrap prime.
    await send_and_check("pkt-A", [0xDEADBEEF, 0xCAFEBABE])

    # Idle gap: long enough for many SYNC insertions (TX emits 1 per 32 idle
    # words) and at least one full SYNC re-prime in the deskew.
    await ClockCycles(tb.dut.hclk, 4000)

    # Packet B — distinct payload; steady-state SYNC re-prime only.
    await send_and_check("pkt-B", [0x12345678, 0x9ABCDEF0])

    # ----- sync_detected APB observability counter (informational) -----------
    # Reads the slave SYNC_DETECTED saturating counter (0x2114 [31:16]) through
    # the full obs chain:
    #   WlinkRxLinkLayer.sync_detected -> Wlink 16b sat-counter (rx-link-clk)
    #   -> axi_chiplet_controller 2-flop apb_clk sync -> region8 slot5 [31:16].
    # This is a FRAMER-level signal (io_link_data == SYNC_WORD, an exact 128-bit
    # compare). It reliably increments when lanes are aligned and the framer
    # samples a bit-exact SYNC — verified =62 in the ZERO-SKEW pair env (full
    # chain proven). Under HEAVY cross-lane word-skew it can read 0 EVEN WHEN DATA
    # DELIVERS INTACT (above): the deskew re-aligns lanes off the per-lane SYNC
    # *slices*, but the framer does not necessarily observe a bit-exact assembled
    # 128-bit SYNC at its single sample point during steady-state re-alignment.
    # DATA round-trip is the functional gate; the counter is logged for HW
    # observability (on silicon a >0 read positively confirms a coherent SYNC).
    s_syncreg = await tb.s_apb.read(APB_R8_SYNC_DET)
    s_sync_cnt = (s_syncreg >> 16) & 0xFFFF
    tb.log.info(f"  [obs] slave SYNC_DETECTED counter (0x2114[31:16]) = "
                f"{s_sync_cnt} (raw 0x{s_syncreg:08x}) — informational under "
                f"skew; the DATA delivery above is the gate")
