...
1. `test_paircredit_nonzero_after_bringup` — regression gate for the credit deadlock <!-- errata 2026-05-29: PAIR_CREDIT_COUNTER != 0 is the WRONG assertion target — it is APB-mirror only. The meaningful credit ledger lives in FCSM at `fe_rx_credit_max` (probed via cocotb hierarchical reference). Agent 7 confirmed `fe_rx_credit_max = 0x1f` on both dies in sim. -->
...
4. **Slave's lane_locked behaviour around training_mode** — per `src/rtl/tidelink_lane_checker_single.sv:195-202`: `match_count` and `locked_pre` are reset on the **training_mode RISING edge**, forcing `locked_o=0` for LOCK_CONSEC cycles. Once `training_mode` returns to 0, `vote_enable` disables but `locked_o` only drops if `match_count` falls. This is expected, NOT a regression. cal_done is the correct post-training health gate. <!-- errata 2026-05-29: prior wording "drops to 0 once training_mode clears" was imprecise — actual reset is on training_mode RISING edge (Agent 4). -->
...
Critical finding: **`HW_SYNC_STATUS` at offset 0x048 bit[0] on the slave is `hw_sync_en_r`** — the **local-FSM-enable mirror of HW_SYNC_CTRL[0]** (per tidelink_ptp.sv:535-539), not a receive counter and not specifically "initiator". If both nodes set HW_SYNC_CTRL[0], both report it. Slave never enabled HW_SYNC, so `HW_SYNC_STATUS=0` is **expected behaviour**, not a bug. <!-- errata 2026-05-29: was "initiator-side enable", corrected per Agent 2 — bit[0] is a local-FSM-enable mirror, symmetric on both nodes. -->

For slave-RX visibility, read instead: <!-- errata 2026-05-29: ptp_rx_valid_r lives at PTP_CTRL @ 0x034 bit[2], not PTP_STATUS @ 0x03C. PTP_STATUS @ 0x03C carries {tx_pending,tx_router_idle}. Per tidelink_ptp.sv:545-548 (Agent 6). -->
- `PTP_CTRL` at offset 0x034 bit[2] = `ptp_rx_valid_r` (per tidelink_ptp.sv:545-548)
- `PTP_RX_PAYLOAD` at offset 0x038 — the received sync payload
- (Note: `PTP_STATUS` at 0x03C carries `{tx_pending, tx_router_idle}`, not RX visibility.)
...
3. **Is Phase A (fix Bug A + Bug B in RTL) gated on the sim regression agent's output?** Probably yes — sim repro is the fastest path to root cause. <!-- errata 2026-05-29: when the sim agent reports credit-ledger probes, the assertion target is FCSM `fe_rx_credit_max != 0` via cocotb hierarchical reference, NOT APB-mirror `PAIR_CREDIT_COUNTER`. See HANDOFF_ERRATA_2026_05_29.md. -->
...
- AUTOCAL_ENABLE = 1'b1 ([tidelink_top.sv:1812](src/rtl/tidelink_top.sv#L1812)) <!-- errata 2026-05-29: line number was 1895, corrected to 1812 (Agent 4) -->
