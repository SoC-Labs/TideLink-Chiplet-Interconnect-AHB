...
Reviewer-1 conditions (cherry-pick 1c9e5e0, verdict SAFE-WITH-CONDITIONS): (1) BUFG count 15→17,
the two new = `u_cnt_bufg`/`u_cap_bufg` in WavD2DGpio `g_shared_cap_bufg`; (2) `cnt_clk_g`/
`cap_clk_g` fan to all 8 `gpiorx_*` ext-clk ports; (3) the fanout-372 general-routing capture LUT
GONE, inter-lane skew ≈0.24 ns; (4) `shared_pad_clk_inv_*` (sel=out_prepend_swi_polarity) present
in the cap_clk_g path (io_pol=1 inverted-edge preserved); (5) run `make asic_v2_elab` + full
sim_gate on the branch (first-ever _v2 elab on the integ base); (6) note: any future Z2 rebuild
inherits this RTL (validated config, but the deployed certified Z2 bitstream predates it).
