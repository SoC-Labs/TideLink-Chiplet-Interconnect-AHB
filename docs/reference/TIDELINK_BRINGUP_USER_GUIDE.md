...
- Full HW test catalogue: [`docs/reference/HW_TEST_SUITE.md`](HW_TEST_SUITE.md), `docs/reference/HW_TEST_SUITE_DEV_LOG.md` _(historical — see git history)_
- ASIC sign-off rules: [`docs/reference/ASIC_TIMING_CONSTRAINTS.md`](ASIC_TIMING_CONSTRAINTS.md)
- Autoneg protocol (I2C role coordination): [`docs/reference/AUTONEG_PROTOCOL.md`](AUTONEG_PROTOCOL.md)
- PHY abstraction (GPIO vs SerDes swap): `docs/archive/PHY_LAYER_ABSTRACTION.md` _(historical — see git history)_
- Most recent interface debug session: [`docs/TIDELINK_INTERFACE_DEBUG_PLAN.md`](TIDELINK_INTERFACE_DEBUG_PLAN.md), `docs/archive/TIDELINK_PHASE0_OBS_20260524_2109.md` _(historical — see git history)_
...
- [ ] **Sign-off gates green** — see `docs/archive/SIGN_OFF_STATUS.md` and the [`ASIC_TIMING_CONSTRAINTS.md §9 checklist`](ASIC_TIMING_CONSTRAINTS.md). Specifically:
  ...
  - CDC sign-off (Spyglass, `docs/reference/SPYGLASS_CDC_SIGNOFF.md`)
  - Per-lane skew determinism metric per `docs/reference/DETERMINISM_VALIDATION.md`
...
**Important:** `role_locked = 1` is necessary but NOT sufficient for traffic. Per `bringup_autocal_i2c.sh:42-47` and [`docs/reference/SHORTCOMINGS.md §14b`](SHORTCOMINGS.md), post-autoneg you still need the `to_data_mode()` step (or equivalent FCSM credit-exchange completion).
...
| 12 | FC TideLink CRC clean | `FC_TIDELINK_CRC_Errors @ 0x44031720` | `0` both sides | FC-layer CRC corruption = physical-layer problem. Escalate to per-lane skew / IDELAY tap / pad map. | New register — was unread until Phase 0 obs 2026-05-24 _(historical — see git history)_ |
...
Cross-ref: docs/archive/TIDELINK_PHASE0_OBS_20260524_2109.md §11 _(historical — see git history)_ for the full diagnostic chain.
...
| Goal | Resource |
|------|----------|
| PHC time sync between master and slave | `pynq_host/scripts/bringup_ptp_sync.sh`, then `bringup_ptp_track_freq.sh`, `bringup_ptp_track_offset.sh`, `bringup_ptp_soak.sh`. See [`docs/reference/PTP_PROTOCOL.md`](PTP_PROTOCOL.md), `docs/archive/PTP_HW_TEST_PLAN.md` _(historical — see git history)_. |
...
| Determinism characterisation | `pynq_host/scripts/determinism_metric.sh` — inter-build per-lane skew variance, per `docs/reference/DETERMINISM_VALIDATION.md` _(historical — see git history)_. |

### 8.2 For ASIC path

| Goal | Resource |
|------|----------|
| STA sign-off | [`docs/reference/ASIC_TIMING_CONSTRAINTS.md §9 checklist`](ASIC_TIMING_CONSTRAINTS.md). Source-sync arcs must be analysed at every PVT corner; per-lane skew bounded by `set_max_delay -datapath_only` + `set_data_check`. |
| CDC sign-off | [`docs/CDC_AUDIT_REPORT.md`](CDC_AUDIT_REPORT.md), `docs/reference/SPYGLASS_CDC_SIGNOFF.md` _(historical — see git history)_. Recovered-RX → core CDC must use the established `tidelink_phc_cdc.sv` SYNC_STAGES pattern. |
...
- Active debug session: [`docs/TIDELINK_INTERFACE_DEBUG_PLAN.md`](TIDELINK_INTERFACE_DEBUG_PLAN.md), `docs/archive/TIDELINK_PHASE0_OBS_20260524_2109.md` _(historical — see git history)_
- Sign-off status snapshot: `docs/archive/SIGN_OFF_STATUS.md` _(historical — see git history)_
- Known shortcomings / open work: [`docs/reference/SHORTCOMINGS.md`](SHORTCOMINGS.md), `docs/archive/OUTSTANDING_WORK_REPORT.md` _(historical — see git history)_
- Bug tracker: [`docs/BUG_TRACKER.md`](BUG_TRACKER.md)
- Repo simplification (deletion candidates): `docs/archive/REPO_SIMPLIFICATION_ASSESSMENT.md` _(historical — see git history)_

---

**End of guide. Keep this in sync with `docs/reference/HW_TEST_SUITE.md` as new test categories are added, and append to Section 5 (Pitfalls) any time a new bringup failure mode is debugged on bench.**
