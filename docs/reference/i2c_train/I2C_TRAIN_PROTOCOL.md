...
**Layer:** Layer 2 of the TideLink bring-up alignment fix (see [`docs/reference/TIDELINK_SPECIFICATION.md §9.10`](../../docs/reference/TIDELINK_SPECIFICATION.md) for the as-built integration; the §2.3 gap from the historical plan is captured in §9.10.1 sub-step ordering. Also [`~/.claude/plans/tidelink-bit-slip-i2c-coordination.md`](../../../.claude/plans/tidelink-bit-slip-i2c-coordination.md) §4 — this document refines those).
...
Steps 4 and 7 must happen **on both peers within a bounded skew**. The autonomous cal FSM (`docs/reference/TIDELINK_SPECIFICATION.md §9.10.1` sub-step 5) handles the per-side timing internally, but it does *not* synchronise entry/exit across the link. The high-speed link itself is not aligned at this point — it cannot be used to coordinate alignment (chicken-and-egg). I²C, the existing sideband, is the natural carrier.
...
- [`docs/reference/TIDELINK_SPECIFICATION.md §9.10`](../../docs/reference/TIDELINK_SPECIFICATION.md) — as-built PHY-Align integration; the historical §2.3 gap is captured under sub-step ordering (§9.10.1).
