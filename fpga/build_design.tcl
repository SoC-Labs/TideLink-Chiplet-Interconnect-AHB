...
# Force GLOBAL (non-OOC) BD synthesis when requested (eth-chiplet). The whole
# multicore+ethernet SoC packaged as one IP has duplicate source BASENAMES
# (two phc_ahb.sv, cmsdk_*, xhb500_*); a per-IP OOC synth flattens them into
# one ipshared/src dir and one clobbers the other (-> "module PHC_AHB not
# found"). synth_checkpoint_mode None emits the BD as plain RTL synthesised
# flat in synth_1 (no per-IP ipshared), the same technique the multicore
# pynq ILA build uses. Bare-link targets keep per-IP OOC (this is opt-in).
if { [info exists ::env(FPGA_BD_GLOBAL_SYNTH)] && $::env(FPGA_BD_GLOBAL_SYNTH) == 1 } {
    set _bdf [get_files -quiet ${design_name}.bd]
    puts "INFO: forcing GLOBAL (non-OOC) BD synthesis (FPGA_BD_GLOBAL_SYNTH=1) on $_bdf"
    set_property synth_checkpoint_mode None $_bdf
}
generate_target all [get_files ${design_name}.bd]
if { [info exists ::env(FPGA_BD_GLOBAL_SYNTH)] && $::env(FPGA_BD_GLOBAL_SYNTH) == 1 } {
    foreach r [get_runs -quiet *nanosoc_eth_chiplet*synth*] { catch { delete_runs -quiet $r } }
}
