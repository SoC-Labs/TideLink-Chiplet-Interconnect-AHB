###-----------------------------------------------------------------------------
### TideLink Chiplet Bridge - Pynq-Z2 Single-Board EXTERNAL Loopback Block Design
...
### The BD is a verbatim clone of pynq-z2-pair-all — same PHC, IDELAY,
### debug-unlock, PMOD-B trigger, address map, strap GPIO. The only
### operational difference is at deploy time: this image is meant to run
### on a SINGLE board with 9 jumper wires bridging the J13 RPi header so
### pad_clk_tx/pad_tx[*] re-enter via pad_clk_rx/pad_rx[*]. SW forces
### master role via the strap GPIO; no peer to negotiate with.
###
### Wired together with pynq-z2-loopback (internal-pad routing): if THIS
### image fails while internal-loopback passes on the same board, the
### bug is in the pad/IDELAY/board-cable layer, not the RTL.
###
### Role: strap GPIO at 0x4404_0000 — SW writes 1 (master) at runtime.
###
