...
###   Rationale: a downstream AHB transaction that never completes (Bug-A
###   class FCSM deadlock, a2l_fifo-full HREADYOUT hang, held replay) used
###   to hard-wedge the PS AXI port — every later access from Linux hangs,
###   SSH dies, board needs a power cycle. The firewall's timeout checks
###   (C_ENABLE_TIMEOUT_CHECKS=1) trip when a channel stalls, after which
###   the firewall completes the wedged transaction (and all subsequent
###   ones) toward the PS with SLVERR — Linux sees a bus error, not a hang.
###   2024.1 catalog has no standalone AXI timeout IP; axi_firewall is the
###   supported wedge-protection primitive for 7-series (zynq:Production).
###
###   Timeouts are RUNTIME registers, not synth parameters. HW reset
###   default = 0xFFFF wait strobes (prescaler=0 -> 1 strobe/hclk, i.e.
###   65535 hclk cycles ~ 10.5 ms @ 6.25 MHz) on every check — already a
###   finite bound. For the target ~10k-cycle trip, bootstrap SW writes
###   0x2710 to the five MAX_*_WAITS regs at CTL+0x30..0x40:
###     +0x000 STATUS (RO)             +0x008 UNBLOCK (W, bit0)
###     +0x030 MAX_CONTINUOUS_RTRANSFERS_WAITS  +0x034 MAX_WRITE_TO_BVALID_WAITS
###     +0x038 MAX_AR_WAITS  +0x03C MAX_AW_WAITS  +0x040 MAX_W_WAITS
###   CTL ports are CROSS-WIRED so the surviving plane can inspect/unblock
###   the tripped one: fw_data CTL on GP0 @0x4406_0000 (the common case —
###   data-path wedge recovered from the control plane), fw_ctrl CTL on
###   GP1 @0x8402_0000. If BOTH planes trip, recovery = reflash.
###   ip2intc_irpt / mi_w_error / mi_r_error are left unconnected by
###   design: glob_irpt_en resets to 0 so the IRQ is inert until SW enables
###   it, and the poll-path is CTL STATUS @+0x000. Wiring them to the IRQ
###   concat would renumber nothing but adds .hwh churn for no consumer.
###
