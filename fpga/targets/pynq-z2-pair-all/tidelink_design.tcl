...
proc create_root_design { parentCell } {
    ...
    #--------------------------------------------------------------------------
    # PTP RE-ADD GATE (Phase 5a, 2026-07-06 feat/ptp-fpga-readd).
    # The PHC (PTP hardware clock) subsystem — phc_0, axi_apb_phc, axi_ahb_ptp —
    # was removed 2026-06-19 (bafd9cd) for slice headroom. This target can now
    # RE-ADD it, gated on env var TIDELINK_FPGA_PTP=1, so the default build stays
    # byte-identical (tie-off path) and only a PTP build pulls the PHC cells back
    # in. When enabled: axi_smc NUM_MI 4 -> 6 (PTP masters take the HIGHEST
    # indices M04/M05 so the existing M00..M03 slave map is untouched); the PHC
    # input tie-off xlconstants are replaced by the real phc_0 IP; apertures
    # ahb_ptp @0x4402_0000 + phc apb @0x4405_0000 are re-asserted. To FIT on the
    # ~84%-slice xc7z020, the PTP build also sets -verilog_define TIDELINK_NO_BEATCAP
    # (build_design.tcl) which drops the 32-entry FCSM per-beat capture ring
    # (~1 KB FF + 32:1 readout mux). Requires the PHC IP repo (FPGA_PHC_IP_REPO,
    # exported by the Makefile via package_phc_ip) on the ip_repo_paths.
    #--------------------------------------------------------------------------
    set ptp_enabled 0
