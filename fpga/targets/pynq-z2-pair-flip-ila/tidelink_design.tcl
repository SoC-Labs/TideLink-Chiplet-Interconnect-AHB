...
proc create_root_design { parentCell } {
    ...
    #--------------------------------------------------------------------------
    # ILA — probe slave's pad_tx[7:0] + pad_clk_tx INTERNALLY (before IOBUF).
    # If Wlink RTL is actually driving these signals, we'll see toggling.
    # If they're stuck low, synthesis dropped the upper TX lanes.
    # Sampled in hclk (50 MHz) so the capture domain is robust.
    #--------------------------------------------------------------------------
    set ila_tx [create_bd_cell -type ip -vlnv xilinx.com:ip:ila:6.2 ila_tx]
    set_property -dict [list \
    ...
    #-- ILA tap on slave's pad_tx side: probe what Wlink IP is actually
    #   driving onto pad_tx[7:0] and pad_clk_tx, sampled in hclk (50 MHz).
    connect_bd_net [get_bd_pins clk_wiz_0/clk_out1]     [get_bd_pins ila_tx/clk]
    connect_bd_net [get_bd_pins tidelink_0/pad_clk_tx]  [get_bd_pins ila_tx/probe0]
    connect_bd_net [get_bd_pins tidelink_0/pad_tx]      [get_bd_pins ila_tx/probe1]
