# TideLink — Pynq-Z2 Paired GPIO-Bridge Target (Target A — mmcmbypass FLIP variant)

**Target A FLIP** variant of `pynq-z2-pair-flip-all` that cuts the
slave-side capacitive load on `pad_clk_rx` by replacing the 8 per-lane
BUFGs inside `WavD2DGpio` with **a single IBUFG + BUFG at the BD level**.
...
cd ~/SoCLabs/tidelink
source set_env.sh
# Target A mmcmbypass FLIP variant — pairs with the straight variant for the
# cross cable. Build this FLIP variant on z2-b and the straight variant on
# z2-a (or whichever way round the ribbon is wired).
FPGA_USE_IDELAY=1 make -C fpga TARGET=pynq-z2-pair-mmcmbypass-flip-all build_design
```

Output bitstream: `imp/fpga/output/pynq-z2-pair-mmcmbypass-flip-all/tidelink.bit`.
