# TideLink — Pynq-Z2 Paired GPIO-Bridge Target (Target A + ODDR centred-launch)

**Target A + ODDR** variant — built on top of `pynq-z2-pair-mmcmbypass-all`.
Adds a **SAME_EDGE ODDR re-launch** of the forwarded TX clock so the
forwarded clock edge on `pad_clk_tx` is **centred** in the `pad_tx[*]` data
eye (UG903 Example Six pattern: `D1=0, D2=1, DDR_CLK_EDGE="SAME_EDGE"`).
The receiver therefore samples the data at the centre of the bit period
instead of right at the edge — the worst sampling point the original
edge-aligned launch from `WavD2DGpioTx.v` line 97 (`assign io_pad_clk =
io_link_clk`) produced. See
[docs/UG903_FORWARDED_CLOCKS_AUDIT_2026_05_28.md](../../../docs/UG903_FORWARDED_CLOCKS_AUDIT_2026_05_28.md)
for the audit + rationale.

Inherits from `pynq-z2-pair-mmcmbypass-all`: the slave-side `pad_clk_rx`
capacitive load is cut by replacing the 8 per-lane BUFGs inside
...
cd ~/SoCLabs/tidelink
source set_env.sh
# Target A + ODDR variant — pairs with the FLIP variant for the cross
# cable. Build the master (this target) on z2-a and the FLIP variant on z2-b.
FPGA_USE_IDELAY=1 make -C fpga TARGET=pynq-z2-pair-mmcmbypass-oddr-all build_design
```

Output bitstream: `imp/fpga/output/pynq-z2-pair-mmcmbypass-oddr-all/tidelink.bit`.
The FLIP variant (mirror RPi pinout) lives in
`fpga/targets/pynq-z2-pair-mmcmbypass-oddr-flip-all/`.
