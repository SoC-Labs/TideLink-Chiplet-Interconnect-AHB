#-----------------------------------------------------------------------------
# TideLink Chiplet Bridge - Pynq-Z2 Paired (STRAIGHT) Source-Synchronous
# Timing Constraints
...
#                                             Vivado infers a BUFG, dedicated
#                                             clock net feeds the gpiorx_*
#                                             capture flops.
#   pad_clk_tx -> Y9  (IO_L14P_T2_SRCC_13)  — single-region clock-capable
#                                             out; forwarded WITH pad_tx[*].
#   (The old comment claiming pad_clk_tx is on "Y6" was STALE — corrected.)
#
...
#
# 2026-05-28 (feat/target-a-oddr): the launching pin is the ODDR clock input
# (clk_tx_oddr/u_oddr/C), NOT the clk_wiz clk_out1. Per UG903 Example Six the
...
# the object-type rule. Discovered via DCP audit of build #9.
set_bus_skew -from [get_pins -hier -filter {NAME =~ "*pad_rx_IBUF[*]/O"}] -to [get_cells -hier -filter {NAME =~ "*gpiorx_*/link_data_pad_clk_reg[*]"}] 2.000
...
# pin (== hclk — see header §[2] and tidelink_design.tcl:200,546). This is
# the same narrow filter pattern used by pad_clk_tx_fwd's -source above,
# so it is known to resolve to exactly one pin. Discovered via DCP audit
# of build #9.
