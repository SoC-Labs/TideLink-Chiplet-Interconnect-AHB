...
### (Wave B2, Target A mmcmbypass variant — single IBUFG+BUFG on pad_clk_rx)
...
###-----------------------------------------------------------------------------
### TARGET A + ODDR (mmcmbypass-oddr-all variant, 2026-05-28):
###   Layered on top of Target A (mmcmbypass-all): the slave-side RX clock
###   path is unchanged (single IBUFG+BUFG via tidelink_clk_rx_buf — ~48 pF
###   -> ~8 pF on the slave pad_clk_rx). ADDITIONAL change here: pad_clk_tx
###   is re-launched through a SAME_EDGE ODDR (tidelink_clk_tx_oddr wrapper)
###   so the forwarded clock edge is CENTRED in the pad_tx[*] data eye
###   instead of edge-aligned to data transitions (the original behaviour
###   of WavD2DGpioTx.v: `assign io_pad_clk = io_link_clk`). Per UG903
###   Example Six, D1=0 + D2=1 with SAME_EDGE emits a clock 180° out of
###   phase with clk_in — the receiver's rising edge of pad_clk_rx lands in
###   the middle of the data bit period.
###
###   See docs/UG903_FORWARDED_CLOCKS_AUDIT_2026_05_28.md and
###   docs/TARGET_A_MMCM_BYPASS_DRAFT_2026_05_28.md for full diagnosis.
###
###   Pairs with the RTL split USE_CLKBUF -> USE_CAP_CLKBUF + USE_LNK_CLKBUF
###   that lands on feat/target-a-rtl (Pieces 1+2). This BD sets
###   USE_CAP_CLKBUF=0 (no per-lane cap BUFG, BD already drives a global
###   clock net) and USE_LNK_CLKBUF=1 (keep BUFG on the derived word clock).
###
...
### pad_clk_rx, etc.) — the 200 Ω external series resistor is on-board and
...
proc create_root_design { parentCell } {
    ...
    # clk_tx_oddr/pad_out drives the BD-level pad_clk_tx port (Y9 master /
    # Y7 slave on the PYNQ-Z2 J13 header).
    #
    # reset is driven by proc_sys_reset_0/peripheral_reset (active-high) so
    # the ODDR is held in a defined state during PS reset.
    #--------------------------------------------------------------------------
