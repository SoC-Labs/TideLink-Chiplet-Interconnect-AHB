# TideLink — Pynq-Z2 Paired GPIO-Bridge Target (Target A + ODDR centred-launch — FLIP)

**Target A + ODDR FLIP** variant — built on top of
`pynq-z2-pair-mmcmbypass-flip-all`. Adds a **SAME_EDGE ODDR re-launch** of
the forwarded TX clock so the forwarded clock edge on `pad_clk_tx` is
**centred** in the `pad_tx[*]` data eye (UG903 Example Six pattern:
`D1=0, D2=1, DDR_CLK_EDGE="SAME_EDGE"`). The receiver therefore samples
the data at the centre of the bit period instead of right at the edge —
the worst sampling point the original edge-aligned launch from
`WavD2DGpioTx.v` line 97 (`assign io_pad_clk = io_link_clk`) produced.
See [docs/UG903_FORWARDED_CLOCKS_AUDIT_2026_05_28.md](../../../docs/UG903_FORWARDED_CLOCKS_AUDIT_2026_05_28.md)
for the audit + rationale.

Inherits from `pynq-z2-pair-mmcmbypass-flip-all`: the slave-side
`pad_clk_rx` capacitive load is cut by replacing the 8 per-lane BUFGs
inside `WavD2DGpio` with **a single IBUFG + BUFG at the BD level**.
See [docs/TARGET_A_MMCM_BYPASS_DRAFT_2026_05_28.md](../../../docs/TARGET_A_MMCM_BYPASS_DRAFT_2026_05_28.md)
for the full diagnosis (~48 pF -> ~8 pF on the slave's `pad_clk_rx` pad).

The two structural changes are **independent and additive**: load
reduction on the RX side + centred-launch on the TX side.
...
**XDC pin assignments are unchanged** from `pynq-z2-pair-flip-all` -- still
the flip-pair pinout (mirrored RPi pins for the cross cable). The 200 ohm
external series resistor on each RPi GPIO is on-board and can't be
removed; this variant only reduces internal load.

Otherwise identical to `pynq-z2-pair-flip-all` -- same address map, same
paired fpgahub flow, same AXI GPIO at `0x4404_0000` whose bit 0 drives
...
cd ~/SoCLabs/tidelink
source set_env.sh
# Target A + ODDR FLIP variant — pairs with the straight (oddr-all) variant
# for the cross cable. Build this FLIP variant on z2-b and the straight
# variant on z2-a (or whichever way round the ribbon is wired).
FPGA_USE_IDELAY=1 make -C fpga TARGET=pynq-z2-pair-mmcmbypass-oddr-flip-all build_design
```

Output bitstream: `imp/fpga/output/pynq-z2-pair-mmcmbypass-oddr-flip-all/tidelink.bit`.
The straight variant (matching RPi pinout) lives in
`fpga/targets/pynq-z2-pair-mmcmbypass-oddr-all/`.
