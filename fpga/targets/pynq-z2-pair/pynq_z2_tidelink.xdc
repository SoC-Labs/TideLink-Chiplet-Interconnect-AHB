...
# Verified against the Vivado pynq-z2 A.0 board file
# (raspberry_pi_tri_i_<idx>): all 18 lanes are RPi GPIO 0..17, all on
# bank 35 at LVCMOS33. The earlier B1-agent map mixed in some bank-13
# pins (V9, U16) that are 1.8 V on the Z2 — those failed Place 30-58.
set_property -dict { PACKAGE_PIN W18 IOSTANDARD LVCMOS33 } [get_ports pad_clk_tx]   ;# RPi GPIO0

# pad_tx[7:0]: TX data lanes (RPi GPIO 1..8)
...
# pad_clk_rx: clock received from paired board (RPi GPIO 9)
set_property -dict { PACKAGE_PIN W10 IOSTANDARD LVCMOS33 } [get_ports pad_clk_rx]   ;# RPi GPIO9

# pad_rx[7:0]: RX data lanes (RPi GPIO 10..17)
