###-----------------------------------------------------------------------------
### TideLink Chiplet Bridge - Pynq-Z2 Single-Instance Block Design TCL (Wave B1)
...
### (Zynq XC7Z020CLG400-1) in single-node configuration. Role strap is
### hardcoded to 0 (slave) for this target.
###
### Design contents:
...
###
### NOTE (role_strap_i):
###   Hardcoded to 1'b0 (slave) in the board wrapper for single-instance.
###   The paired target (pynq-z2-pair) overrides this via an AXI GPIO.
###
### NOTE (ahb_mng):
...
proc create_root_design { parentCell } {
    ...
    #--------------------------------------------------------------------------
    # AXI SmartConnect: 1 PS master -> 6 slaves
    #   M00 -> axi_ahb_sub
    #   M01 -> axi_ahb_tx
    #   M02 -> axi_ahb_fifo
    #   M03 -> axi_ahb_ptp
    #   M04 -> axi_apb
    #   (M05 reserved — see ahb_mng note in header)
    #--------------------------------------------------------------------------
