###-----------------------------------------------------------------------------
### nanosoc_compute_chiplet - Vivado IP Packaging Script
...
###-----------------------------------------------------------------------------
### Packages nanosoc_compute_chiplet_vivado_wrapper (the WHOLE multicore compute
### SoC + TWO internal TideLinks + one TideChart) as a Vivado IP for the KR260
### compute-chiplet target. Two-link sibling of package_eth_chiplet_ip.tcl.
###
### This is also the SCOPING SYNTH: ipx::package_project runs OOC elaboration of
### the entire SoC on xck26, the first proof of whether the design fits/synthesises.
### Expect it to surface FPGA memory-model gaps (behavioural SRAM -> BRAM) — that
### is the intended scoping outcome.
###
### Required environment variables:
###   FPGA_COMPONENT_FILELIST     - Path to the compute-chiplet Vivado filelist tcl
###   FPGA_COMPONENT_LIB          - Output directory for packaged IP component.xml
###   NANOSOC_COMPUTE_CHIPLET_DIR - Parent repo root (used by the filelist)
###   SOCLABS_TIDELINK_DIR        - tidelink submodule root
###   FPGA_VENDOR / FPGA_CORE_REV - optional IP metadata
###-----------------------------------------------------------------------------

# Vivado message gate — mirror package_eth_chiplet_ip.tcl / build_design.tcl
...
# STEP 0: Read design sources (the parent-repo SoC + 2x tidelink + tidechart + integration)
source $env(FPGA_COMPONENT_FILELIST)

# Add the wrapper (next to this script)
set wrapper_dir [file dirname [info script]]
read_verilog $wrapper_dir/nanosoc_compute_chiplet_vivado_wrapper.v
...
set_property display_name        "nanoSoC Compute Chiplet (dual TideLink D2D)" $core
set_property description          "Full nanoSoC multicore compute SoC + TWO internal TideLink die-to-die links + one TideChart, for the KR260 two-board demo. ps_ahb_s AHB backdoor (G2), two GPIO-PHY ribbons (link 1 tied off), full CoreSight SWJ-DP, TideChart interrupt." $core
...
# user_ref / phc clocks are board-specific; skip static FREQ_HZ DRC
...
# STEP 4: Memory map for the one addressable slave (ps_ahb_s).
...
add_memory_map_32 ps_ahb_s 4294967296 $core
