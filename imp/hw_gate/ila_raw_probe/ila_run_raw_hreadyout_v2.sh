...
OUT="${OUTDIR:-$HG/ila_raw_probe/run2}"; mkdir -p "$OUT"
...
echo "==== RAW HREADYOUT ILA CAPTURE (V2) START $(date -u) ===="
...
timeout 60 scp -q -o BatchMode=yes "$HG/ila_raw_probe/ila_capture_raw_hreadyout_v2.tcl" mapstone-dev:/tmp/ila_capture.tcl || { echo "tcl stage FAILED" | tee -a "$SUM"; exit 1; }
...
MDEV "rm -f /tmp/ila_armed /tmp/ila_done /tmp/ila_fail /tmp/do_capture /tmp/ila_trigged /tmp/ila_capture.csv /tmp/ila_capture_trig.csv; cd /tmp && nohup timeout 900 /tools/Xilinx/2025.2/Vivado/bin/vivado -mode batch -notrace -source /tmp/ila_capture.tcl >/tmp/ila_capture.log 2>&1 & echo launched" 25
armed=0
...
# ---- stimulus, in ROUNDS, checking for a REAL trigger between them --------
# Round 1 of this campaign fired once, waited 25s, then force-captured -- and got
# a window with wr_hold_r=0 and sub_stall_ctr_r frozen at 0, i.e. VOID on the
# pre-registered guard. Two changes here:
#   * the v2 tcl uploads the TRIGGERED window before any re-arm, so a wr_hold_r
#     pulse that has already come and gone is still readable (/tmp/ila_trigged);
#   * if a round produces no trigger AND die_a is still responsive, fire again
#     with a heavier write burst rather than force-capturing an idle wrapper.
# Once die_a's PS is wedged no further stimulus is possible, so the loop stops.
trigged=0; wedged=0; rounds=0
for r in 1 2 3; do
  rounds=$r
  echo "-- stimulus round $r --"
  ...
  n=$(( 32 * r ))
  ( bk $IP_A "python3 scripts/kr260_eth_soak_fwd.py write $n 0xB0008000" 150 >/dev/null 2>&1 ) &
  echo "  provoke write ($n words) fired in bg; settling"
  sleep 20
  # Spaced deliberately: ~25 back-to-back ssh sessions trips sshd rate limiting,
  # and the next command then reads "Connection reset by peer", which is
  # INDISTINGUISHABLE from a wedged board.
  if MDEV "ls /tmp/ila_trigged 2>/dev/null" 15 | grep -qa ila_trigged; then
    trigged=1; echo "  TRIGGER FIRED in round $r -- wr_hold_r reached 1, window captured" | tee -a "$SUM"
    break
  fi
  echo "  no trigger yet in round $r"
  if bk $IP_A "python3 scripts/eth_tlapb_poke.py read 0x2010" 30 | grep -qaE '0x[0-9a-fA-F]'; then
    echo "  die_a still responsive after round $r"
  else
    wedged=1; echo "  die_a UNRESPONSIVE after round $r -- no more stimulus possible" | tee -a "$SUM"
    break
  fi
done
# One last look for a late trigger before we force and destroy the armed window.
sleep 15
MDEV "ls /tmp/ila_trigged 2>/dev/null" 15 | grep -qa ila_trigged && { trigged=1; echo "TRIGGER FIRED (late)" | tee -a "$SUM"; }
echo "stimulus rounds=$rounds  trigger_fired=$trigged  die_a_unresponsive=$wedged" | tee -a "$SUM"
# Ask for the forced window LAST (it re-arms the core and destroys the triggered
# one -- which the tcl has already uploaded and written by this point).
...
# Final responsiveness read, AFTER the capture so it cannot delay it.
if bk $IP_A "python3 scripts/eth_tlapb_poke.py read 0x2010" 30 | grep -qaE '0x[0-9a-fA-F]'; then
  echo "die_a responsive at capture time: YES (may not have been wedged)" | tee -a "$SUM"
else
  echo "die_a responsive at capture time: NO (obs read timed out = wedged)" | tee -a "$SUM"
fi
...
timeout 90 scp -q -o BatchMode=yes mapstone-dev:/tmp/ila_capture_trig.csv "$OUT/ila_capture_trig.csv" 2>/dev/null &&   echo "TRIG CSV: $(wc -l < "$OUT/ila_capture_trig.csv") lines" | tee -a "$SUM" ||   echo "TRIG CSV: none (trigger never fired)" | tee -a "$SUM"
timeout 90 scp -q -o BatchMode=yes mapstone-dev:/tmp/ila_capture.csv "$OUT/ila_capture.csv"
MDEV "grep -aE 'TRIGGERED=|TRIG_CSV_WRITTEN|DO_CAPTURE|TRIG_STATUS|CAPTURE_DONE|WARN' /tmp/ila_capture.log" 25 | mask | tee -a "$SUM"
echo "CSV: $(wc -l < "$OUT/ila_capture.csv" 2>/dev/null) lines" | tee -a "$SUM"
echo "RESULT: capture complete, csv at $OUT/ila_capture.csv" >> "$SUM"
echo "==== RAW HREADYOUT ILA CAPTURE (V2) END $(date -u) ===="
