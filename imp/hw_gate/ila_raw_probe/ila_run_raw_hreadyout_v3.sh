...
OUT="${OUTDIR:-$HG/ila_raw_probe/run3}"; mkdir -p "$OUT"
...
echo "==== RAW HREADYOUT ILA CAPTURE (V3) START $(date -u) ===="
...
timeout 60 scp -q -o BatchMode=yes "$HG/ila_raw_probe/ila_capture_raw_hreadyout_v3.tcl" mapstone-dev:/tmp/ila_capture.tcl || { echo "tcl stage FAILED" | tee -a "$SUM"; exit 1; }
...
# ---- stimulus: ONE round, then leave the core armed and UNTOUCHED ---------
# V2 polled the armed core with `wait_on_hw_ila` to detect a trigger. That call
# returned success on a core that had not triggered, the upload logged
# "Labtools 27-157 ... stopped. No data to upload", and the harness printed
# "TRIGGER FIRED" on a 2-line header-only CSV. Poking the core that early may
# also have STOPPED it, so nothing was armed while the stimulus ran. V3 issues
# no JTAG operation at all between arming and the capture: the trigger gets the
# whole stimulus window, and "did it fire?" is answered from CSV CONTENT.
...
( bk $IP_A "python3 scripts/kr260_eth_soak_fwd.py write ${SOAKN:-128} 0xB0008000" 180 >/dev/null 2>&1 ) &
echo "provoke write (${SOAKN:-128} words) fired in bg; core armed, waiting ${ARMWAIT:-75}s"
sleep "${ARMWAIT:-75}"
...
echo "==== RAW HREADYOUT ILA CAPTURE (V3) END $(date -u) ===="
