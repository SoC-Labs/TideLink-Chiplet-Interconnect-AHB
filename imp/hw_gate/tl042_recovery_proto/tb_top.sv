...
`endif
    ...
    // ========================================================================
    // TL-042 PROTOTYPE wedge injector (guarded; only in the tl042_recovery_proto
    // build). cocotb DEPOSITS the per-signal enables/values below to hold the
    // MASTER die's DOWNSTREAM (Wlink-driven) s_axi handshakes at chosen values:
    //   * awready/wready forced LOW  = the AW/W-not-accepted write-wedge
    //   * bvalid_ctrl/rvalid forced LOW = keep the (untrained) link's B/R quiet so
    //     no X leaks into the bridge B/R path (the recovery is the ONLY B source)
    //   * setting f_awready_val/f_wready_val HIGH while still forced = "link revives"
    //     (used by the double-accept edge-case test)
    // The force targets are the Wlink-driven nets s_axi_awready / s_axi_wready /
    // s_axi_bvalid_ctrl / s_axi_rvalid, which exist UNCHANGED under both the
    // control (unpatched) and fix (patched) tidelink_top.
    // ========================================================================
    `ifdef TB042_WEDGE_HOOK
    reg f_awready_en  = 1'b0;  reg f_awready_val = 1'b0;
    reg f_wready_en   = 1'b0;  reg f_wready_val  = 1'b0;
    reg f_bctrl_en    = 1'b0;
    reg f_rvalid_en   = 1'b0;
    always @(f_awready_en or f_awready_val)
        if (f_awready_en) force u_master.s_axi_awready = f_awready_val;
        else              release u_master.s_axi_awready;
    always @(f_wready_en or f_wready_val)
        if (f_wready_en)  force u_master.s_axi_wready = f_wready_val;
        else              release u_master.s_axi_wready;
    always @(f_bctrl_en)
        if (f_bctrl_en)   force u_master.s_axi_bvalid_ctrl = 1'b0;
        else              release u_master.s_axi_bvalid_ctrl;
    always @(f_rvalid_en)
        if (f_rvalid_en)  force u_master.s_axi_rvalid = 1'b0;
        else              release u_master.s_axi_rvalid;
    `endif
