...
#
# Board credentials/addresses come from the environment (no secrets in-repo):
#   KR260_PASSWORD  (required) — the board sudo/ssh password
#   DIE_A / DIE_B   (optional) — ssh user@host for each die (lab-internal defaults)
set -u
PW=${KR260_PASSWORD:?set KR260_PASSWORD to the board password (not stored in-repo)}
