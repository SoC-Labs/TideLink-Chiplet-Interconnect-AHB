#!/bin/bash
#
# FIX VARIANT (2026-05-25): see sw_coord_autocal_region8.sh for the original.
# The original `to_data_mode()` swreset cycle (0x27f08 -> 0x27f00 -> 0x27f07)
# transiently dropped swi_enable to 0, which per FC.scala:619 resets ALL 7
# FCSMs to IDLE and clears the cr_pkt_seen_rx/crack_pkt_seen_rx sticky bits.
# After swi_enable returned high, both sides had to re-race the credit
# handshake from scratch — and on HW (2026-05-24 bridge1 test) one side
# observably stalled at IDLE while the other reached SEND_CREDITS1, leaving
# the doorbell unable to cross.
#
# Fix: hold swi_enable high throughout the swreset cycle:
#   0x27f08 -> 0x27f09  (assert swreset, swi_enable stays 1)
#   0x27f00 -> 0x27f01  (deassert swreset, swi_enable stays 1)
#   0x27f07 unchanged   (full enable, swreset off)
#
# See: docs/TIDELINK_PHASE0_OBS_20260524_2109.md §9
#
# Original below (for diff visibility):
...
time.sleep(0.005)
struct.pack_into(\"<I\", w, wo+0x208, 0x00027f09)  # FIX: swreset+swi_enable (was 0x27f08 - bug dropped swi_enable, FC.scala:619 resets all 7 FCSMs to IDLE)
time.sleep(0.005)
struct.pack_into(\"<I\", w, wo+0x208, 0x00027f01)  # FIX: swreset off, keep swi_enable high (was 0x27f00 - same bug)
time.sleep(0.005)
struct.pack_into(\"<I\", w, wo+0x208, 0x00027f07)  # swi+lltx+lltx_1 enabled (unchanged)
'" 2>/dev/null
}
