...
`endif
  ...
  // tx_link_clk domain and syncs to apb_clk internally. Pure RO fan-out;
  // UNCONDITIONAL (like the sibling winscan obs) so it survives Vivado IP
  // packaging where a define never reaches the packaged-IP OOC synth. apb_clk
  // domain -> read directly by the Region F mux.
  ,
...
`endif
  ...
  // NO datapath change; UNCONDITIONAL (like the sibling winscan obs) so it
  // survives Vivado IP packaging where a define never reaches OOC synth.
  // -------------------------------------------------------------------------
