...
module tidelink_ptp #(
    ...
    // defence — `(* keep *) (* dont_touch *)` prevent Vivado from splitting
    ...
    // (tx_state_r=TX_IDLE, valid=0). Build #19 (99c9d58) repaired the
    // master TX with the (valid && ready) handshake gate; this rebase
    // re-applies Agent Q's slave-side defence on top so the now-functional
    // master TX can be evaluated against the now-defended slave RX gate.
        ...
        if (!hresetn) begin
            ...
            case (tx_state_r)
                ...
                TX_SEND: begin
                    ...
                    // alone. If a replicated ptp_enable_r=0 copy gated
                    // ptp_sp_tx_valid low (builds #14-17 master-TX-stuck
                    // symptom), the original `ready`-only check would
                    // silently "complete" without ever asserting valid on
                    // the wire. See docs/PHC_PHASE1_HW_REPORT.md §"Build #19".
