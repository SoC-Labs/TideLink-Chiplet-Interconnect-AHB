...
puts $mf "6. **Chip-top ECO required — residual routing DRCs.** \`fc_drc\` reports 9"
puts $mf "   End-Of-Line spacing violations clustered in the std-cell band shadowing"
puts $mf "   the rf_16k macro (bottom-right corner). The in-block 3-pass route_eco"
puts $mf "   loop took the count 99 → 9 (91%); the residuals are structural at the"
puts $mf "   macro-shadow boundary (router can't escape local congestion without"
puts $mf "   re-placement, which would risk timing). Chip-top assembly should"
puts $mf "   re-route this region after the partition is stitched, or accept the"
puts $mf "   residual if EOL waivers are negotiated with the foundry. Detail per"
puts $mf "   violation: \`reports/07_check_routes.rep\`."
puts $mf "7. **PG-connectivity disposition — PASS\\*** (\`reports/07_summary.rep\`):"
puts $mf "   check_pg_connectivity reports 4622 floating std-cells which the"
puts $mf "   independent net-centric audit (\`reports/08_pg_deepdive.rep\`)"
puts $mf "   characterises as trim:true PG-mesh wire-stub-fragment artefact — 0"
puts $mf "   logical floats. Supporting evidence: check_pg_drc PASS, timing closed"
puts $mf "   on both setup and hold, Formality LEC clean. Same artefact mechanism"
puts $mf "   characterised by ahb_qspi's INTEGRATION_CHANGES.md PG deep-dive."
puts $mf "8. LEC: \`cd ../formality && make lec\` for RTL→netlist equivalence check."
